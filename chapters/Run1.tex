\chapter{Search for the Standard Model production of four top quarks at $\sqrt{s} =$ 8TeV }
\label{c:Run1}
...
The signal SM \tttt Monte Carlo (MC) samples and the background MC samples are given in \fxnote*{add table appendix}{Table X}, along with the MC generator used to produce these samples, the order at which they were produced and the number of events produced. MC samples were produced for each systematic, these can be found in \fxnote*{add table appendix}{Table X}.
...
\item At least 2 \fxnote*{CSVM defined in event reco}{CSVM tagged b-jets}
\item $\HT > $ 400 \GeV 
\item $\MET > $ 30 \GeV 
\end{itemize}
...
It is assumed that the multi-jet background can be estimated from the data minus the background expectation from all of the other MC samples considered in Section~\ref{sec:datasimulation}. The results of the background subtraction from data and the defined ABCD regions are shown in Fig.~\ref{fig:QCDplots}.
